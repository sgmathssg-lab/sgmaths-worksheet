...
def grid_parallel_lines_abcdef(w=200, h=190):
    ...
    # Pentagon vertices + interior point F
    # ABCDE is closed pentagon; F is on the left side AE interior
    # AF // ED (both horizontal segments on the grid)
    pts = {
        "A": (gx + 0*step, gy + 4*step),   # left mid
        "B": (gx + 3*step, gy + 8*step),   # top centre
        "C": (gx + 7*step, gy + 4*step),   # right mid
        "D": (gx + 5*step, gy + 0*step),   # bottom right
        "E": (gx + 0*step, gy + 0*step),   # bottom left
        "F": (gx + 3*step, gy + 4*step),   # interior, same row as A: AF // ED
    }

    # Draw enclosed pentagon A-B-C-D-E-A
    for p1, p2 in [("A","B"),("B","C"),("C","D"),("D","E"),("E","A")]:
        x1,y1 = pts[p1]; x2,y2 = pts[p2]
        d.add(Line(x1, y1, x2, y2, strokeColor=BLACK, strokeWidth=1.4))

    # Draw interior line AF (AF // ED — both horizontal)
    x1,y1 = pts["A"]; x2,y2 = pts["F"]
    d.add(Line(x1, y1, x2, y2, strokeColor=BLACK, strokeWidth=1.4))

    # Parallel tick marks on AF
    af_mid_x = (pts["A"][0] + pts["F"][0]) // 2
    af_y = pts["A"][1]
    d.add(Line(af_mid_x - 4, af_y + 5, af_mid_x + 4, af_y + 5,
               strokeColor=BLACK, strokeWidth=1.2))
    d.add(Line(af_mid_x - 4, af_y + 5, af_mid_x + 4, af_y + 5,
               strokeColor=BLACK, strokeWidth=1.2))

    # Parallel tick marks on ED
    ed_mid_x = (pts["E"][0] + pts["D"][0]) // 2
    ed_y = pts["E"][1]
    d.add(Line(ed_mid_x - 4, ed_y + 5, ed_mid_x + 4, ed_y + 5,
               strokeColor=BLACK, strokeWidth=1.2))
