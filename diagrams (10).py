...
def grid_parallel_lines_abcdef(w=200, h=190):
    ...
    # Parallel tick marks on AF (horizontal, top)
    af_mid_x = (pts["A"][0] + pts["F"][0]) // 2
    af_y = pts["A"][1]
    d.add(Line(af_mid_x - 5, af_y + 5, af_mid_x + 5, af_y + 5,
               strokeColor=BLACK, strokeWidth=1.2))

    # Parallel tick marks on ED (horizontal, bottom)
    ed_mid_x = (pts["E"][0] + pts["D"][0]) // 2
    ed_y = pts["E"][1]
    d.add(Line(ed_mid_x - 5, ed_y + 5, ed_mid_x + 5, ed_y + 5,
               strokeColor=BLACK, strokeWidth=1.2))
